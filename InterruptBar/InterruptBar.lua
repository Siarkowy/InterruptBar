...
local function InterruptBar_UpdateText(text,cooldown)
	if cooldown > 60 then
        text:SetFormattedText("%d'", ceil(cooldown/60))
...
local function InterruptBar_StopAbilityIfEnabled(ability)
    if activetimers[ability] then
        InterruptBar_StopAbility(activetimers[ability],ability)
    end
end
...
local function InterruptBar_COMBAT_LOG_EVENT_UNFILTERED(...)
	...
	return function(_, eventtype, _, srcName, srcFlags, _, dstName, dstFlags, id)
        ...
        if spellID == 11958 then -- Cold Snap
            InterruptBar_StopAbilityIfEnabled(GetSpellInfo(27088)) -- Frost Nova
            InterruptBar_StopAbilityIfEnabled(GetSpellInfo(45438)) -- Ice Block
            InterruptBar_StopAbilityIfEnabled(GetSpellInfo(31687)) -- Summon Water Elemental
        elseif spellID == 14185 then -- Preparation
            InterruptBar_StopAbilityIfEnabled(GetSpellInfo(26889)) -- Vanish
            InterruptBar_StopAbilityIfEnabled(GetSpellInfo(36554)) -- Shadowstep
        end

        -- Spell replacement
        if spellID == 32182 then spellID = 2825 -- Heroism --> Bloodlust
        elseif spellID == 6789 or spellID == 17925 or spellID == 17926 then spellID = 27223 -- Death Coil ranks
        elseif spellID == 19505 or spellID == 19731 or spellID == 19734 or spellID == 19736 or spellID == 27276 then spellID = 27277 -- Devour Magic ranks
        elseif spellID == 19244 then spellID = 19647 -- Spell Lock ranks
		elseif spellID == 49376 then spellID = 16979; useSecondDuration = true end -- Feral Charge - Cat -> Feral Charge - Bear
